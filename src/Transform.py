...
def get_args():
    ...
    parser.add_argument(
        "-ni",
        "--no_img",
        help="flag to omit image",
        action="store_true"
    )
    parser.add_argument(
        "-nh",
        "--no_histogram",
        help="flag to omit histogram",
        action="store_true"
    )
...
def main():
    ...
    for transformed_root in transformed.keys():
        ...
        for entry in transformed[transformed_root]:
            ...
            landmark_img = generate_landmarks(
                entry["final"].copy(),
                entry["landmarks"]
                )
            ...
            if not args.no_img:
                ...
                pcv.print_image(
                    entry["input"],
                    filename=f"{final_out_path}/{filename}"
                    )
                pcv.print_image(
                    entry["blur"], filename=f"{final_out_path}/blur_{filename}"
                )
                pcv.print_image(
                    entry["masked"],
                    filename=f"{final_out_path}/mask_{filename}"
                )
                pcv.print_image(
                    entry["final"],
                    filename=f"{final_out_path}/final_{filename}"
                )
                pcv.print_image(
                    entry["size"],
                    filename=f"{final_out_path}/size_{filename}"
                )
                cv2.imwrite(
                    f"{final_out_path}/landmark_{filename}",
                    cv2.cvtColor(landmark_img, cv2.COLOR_RGB2BGR),
                )
                cv2.imwrite(f"{final_out_path}/roi_{filename}", entry["roi"])

            if not args.no_histogram:
                entry["hist_before"].save(
                    f"{final_out_path}/hist_before_{filename}.png"
                )
                entry["hist_after"].save(
                    f"{final_out_path}/hist_after_{filename}.png"
                    )
