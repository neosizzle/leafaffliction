...
def main():
    ...
    for transformed_root in transformed.keys():
        for entry in transformed[transformed_root]:
            ...
            pcv.print_image(
                entry["input"], filename=f"{public_img_path}/input_{id}.png"
            )
            pcv.print_image(
                entry["final"],
                filename=f"{public_img_path}/transformed_{id}.png"
            )
            id += 1
    ...
    def numpy_converter(obj):
        if isinstance(obj, np.generic):
            return obj.item()
        raise TypeError(f"{obj.__class__} is not JSON serializable")
    ...
    with open(f"{render_json_out_path}/pred.json", "w") as f:
        json.dump(
            ...
            f,
            indent=4,
            default=numpy_converter
            )

    os.execve(
        sys.executable,
        [sys.executable, "-m", "http.server", "8080", "--directory", "public"],
        os.environ,
    )
