...
class Transformer:
    ...
    def image_process(self, image_path):
        ...
        # is needed to make a binary image,
        # which is required for object segmentation (bg removal)
        # this method is converting RGB to HSV
        # and extract the saturation channel
        ...
        # Use another channel (LAB) to do
        # grayscaling and join with the origninal
        ...
        roi = pcv.roi.rectangle(
            img=ab_fill, x=roi_x, y=roi_y, h=roi_height, w=roi_width
        )
        ...
        left_landmarks, right_landmarks, center_h = y_plm(
            img=masked2, mask=roi_mask
        )
        top_landmarks, btm_landmarks, center_w = x_plm(
            img=masked2, mask=roi_mask
        )
        landmark_obj = {
        ...
        if path.is_dir():
            ...
            for root, dirs, files in os.walk(path):
                ...
                for i in range(N_THREADS):
                    t = threading.Thread(
                        target=process_files_in_thread,
                        args=(i, split_files[i])
                    )
                    threads.append(t)
                    t.start()
