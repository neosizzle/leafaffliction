...
def plot_diagrams(path, distrib):
	...
	fig = make_subplots(rows=1, cols=2, specs=[[{"type": "bar"}, {"type": "pie"}]],
						subplot_titles=("Histogram of Class Frequencies", "Pie Chart of Class Frequencies"), column_widths=[0.6, 0.4])

	fig.add_trace(go.Bar(x=df['Class'], y=df['Frequency'], name='Frequency'), row=1, col=1)

	fig.add_trace(go.Pie(labels=df['Class'], values=df['Frequency'], name='Frequency'), row=1, col=2)

	fig.update_layout(title_text="Class Frequency Visualization: Histogram and Pie Chart")
...
def get_args():
	...
	parser.add_argument('-o', '--output_path', help='optional report output path', type=str, default=None)
